      ...
      format.xlsx do
        ...
        wb.styles do |s|
          ...
          wb.add_worksheet(name: 'Resultado') do |sheet|
            ...
            sheet.column_widths *([9, 56, 17, 15] + (4..@headers.size-4).map{|i| i % 2 == 0 ? 7 : 7} + [10, 10, 7])
            ...
            sheet.col_style 3, name_campus_cell, :row_offset => 4
            (4..@headers.size-4).each do |i|
              if i % 2 == 0
            ...
            sheet.col_style @headers.size-2, general_grade_cell, :row_offset => 4
        ...
        send_data pkg.to_stream.read, :filename => "#{(@exam.exam_datetime || @exam.exam_executions.first.datetime).strftime('%d_%m_%Y')}_#{@exam.exam_product_years.map(&:name).join(', ')}_#{DateTime.now.strftime('%Y%m%d%H%M%S')}.xlsx", :type => "application/vnd.openxmlformates-officedocument.spreadsheetml.sheet"
