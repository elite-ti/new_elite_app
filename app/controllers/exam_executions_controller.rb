  ...
  def result
    ...
    respond_to do |format|
      ...
      format.xlsx do
        ...
        send_data ResultReportHelper.exam_execution_report(@titles, @headers, @output), :filename => "#{@exam_execution.datetime.strftime('%d_%m_%Y')}_#{@exam_execution.super_klazz.name}_#{DateTime.now.strftime('%Y%m%d%H%M%S')}.xlsx", :type => "application/vnd.openxmlformates-officedocument.spreadsheetml.sheet"
