...
class TempCardProcessing
  ...
  def initialize (path, executable_path)
    @original_path = path
    @status = BEING_PROCESSED_STATUS
    @lines = []
    @run_path = executable_path
    if File.exists? File.join(File.dirname(path), "submission.csv")
      @parameters = {}
      File.open(File.join(File.dirname(path), "submission.csv")).each_line{ |s|
        @parameters[s.split(',')[0]] = s.split(',')[1..-1]
      }
    else
...
folder_path = '/Users/pauloacmelo/Dropbox/JotForm/SCHOOL_CARDS_P4' # TOFILL
...
print "=> Renaming all pdf files to submission.pdf\n"
p ""
Find.find(folder_path).each do |file|
  next if File.basename(file) == 'submission.pdf'
  next if file.index(/.pdf/).nil?
  p "Renamed #{file} to 'submission.pdf'"
  File.rename(file, File.join(File.dirname(file), 'submission.pdf'))
end
...
Dir.new(folder_path).each do |submission_path|
  ...
  next if File.exists?(File.join(folder_path, submission_path, 'output.csv'))
  puts submission_path
  Dir.new(File.join(folder_path, submission_path)).each do |compacted_file_path|
    # p "#{File.join(folder_path, submission_path, compacted_file_path)}"
