...
folder_path = '//home/elite/Desktop/test' # TOFILL
...
`find '#{folder_path}' -name '*_normalized.tif' -exec rm '{}' \\;`
...
Find.find(folder_path) do |file_path|
  ...
  normalized_path = File.join(File.dirname(file_path), 
    File.basename(file_path, file_to_be_processed_format) + '_normalized.tif')
